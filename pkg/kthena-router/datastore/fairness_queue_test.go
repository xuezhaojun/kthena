...
func TestBackpressure_InflightLimitBlocksDequeue(t *testing.T) {
	// With 2 pods, at most 2 requests can be inflight at a time.
	// Further requests are blocked until a Release() is called.
	...
	pq.SetPodCounter(func() int { return 2 }) // 2 pods => max 2 inflight
	defer pq.Close()

	now := time.Now()
	req1 := &Request{ReqID: "req-1", UserID: "u1", ModelName: "m", Priority: 1.0, RequestTime: now, NotifyChan: make(chan struct{})}
	req2 := &Request{ReqID: "req-2", UserID: "u2", ModelName: "m", Priority: 2.0, RequestTime: now.Add(time.Millisecond), NotifyChan: make(chan struct{})}
	req3 := &Request{ReqID: "req-3", UserID: "u3", ModelName: "m", Priority: 3.0, RequestTime: now.Add(2 * time.Millisecond), NotifyChan: make(chan struct{})}
	...
	// First 2 requests should be dequeued (inflight limit = 2)
	select {
	...
	// Third request should be blocked (inflight=2 >= maxInflight=2)
	select {
	...
	if pq.GetInflightCount() != 2 {
		t.Errorf("Expected inflight=2, got %d", pq.GetInflightCount())
	}

	// Release one request -> should immediately unblock req3
	req1.Release()

	select {
	case <-req3.NotifyChan:
		// Success: released slot allowed req3 through
	case <-time.After(500 * time.Millisecond):
		t.Fatal("Timeout: req-3 should be dequeued after Release()")
	}
}

func TestBackpressure_BlockedByBackendMetrics(t *testing.T) {
	// Even with inflight < pod count, if backend metrics show all pods busy,
	...
	pq := NewRequestPriorityQueueWithConfig(nil, cfg, nil, checker)
	pq.SetPodCounter(func() int { return 4 }) // 4 pods
	defer pq.Close()
	...
	// Request should NOT be dequeued since backend reports no capacity
	select {
	case <-normalReq.NotifyChan:
		t.Fatal("Request should be blocked by backpressure (backend metrics)")
	case <-time.After(200 * time.Millisecond):
		// Good: request remains blocked
	}

	// Now simulate backend getting capacity (metrics refresh)
	backendHasCapacity = true

	// Request should now be dequeued
	select {
	case <-normalReq.NotifyChan:
		// Success: dequeued after backend metrics show capacity
	case <-time.After(500 * time.Millisecond):
		t.Fatal("Timeout: request should be dequeued after backend gains capacity")
	}
}

func TestBackpressure_ReleaseTriggersImmediateDequeue(t *testing.T) {
	// Verify that when Release() is called, the next request is dequeued immediately
	...
	cfg := FairnessQueueConfig{
		...
		MaxQPS:                   100,
		TokenWeight:              1.0,
		BackpressurePollInterval: 5 * time.Second, // Very slow tick to prove release-driven dequeue
	}
	pq := NewRequestPriorityQueueWithConfig(nil, cfg, nil, checker)
	...
	// First request should dequeue on the first tick
	select {
	case <-req1.NotifyChan:
	...
	// req2 should be blocked (inflight=1, max=1)
	select {
	case <-req2.NotifyChan:
		t.Fatal("req-2 should be blocked")
	case <-time.After(100 * time.Millisecond):
		// Good
	}

	// Release req1 -> req2 should dequeue IMMEDIATELY (not waiting 5s for next tick)
	start := time.Now()
	req1.Release()
	...
	case <-req2.NotifyChan:
		...
		if elapsed > 500*time.Millisecond {
			t.Errorf("Release-driven dequeue too slow: took %v, expected <500ms", elapsed)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("Timeout: req-2 should be dequeued immediately after Release()")
	}
}
